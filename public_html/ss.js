...
/* global APPLICATION_FULL_TITLE_DEFAULT */
/* global ClientId.length */
...
/* global Map */
/* global MAXIMUM_NUMBER_OF_ALLOWED_CLOSED_TABS */
...
/* global devicePixelRatio, Tab, HTMLOptGroupElement */
...
function checkRequiredArgumentFull(argument, name, checker, dataTypeName) {
    ...
    if (!(checker instanceof Function))
        throw new TypeError(createStringFromTemplate(COMMON_TEXT_LIST.invalidArgumentDataTypeTemplate, "checker", "Function"));

    if (isNullOrUndefined(argument))
        throw new TypeError(createStringFromTemplate(COMMON_TEXT_LIST.invalidArgumentCannotBeNullorUndefinedTemplate, name));

    if (!checker(argument))
        throw new TypeError(createStringFromTemplate(COMMON_TEXT_LIST.invalidArgumentDataTypeTemplate, name, dataTypeName));
...
function checkRequiredString(argument, name) {
    return checkRequiredArgumentFull(argument, name, isString, "string");
}
/**
 * @description Checks the specified string argument and throws an error if the string is not null ot undefined and not approved by the specified checker.
 * @param {Number} argument The string argument.
 * @param {String} name The name of the argument.
 * @returns {Object} The specified string.
 * @public
...
function checkOptionalString(argument, name) {
    return checkOptionalArgumentFull(argument, name, isString, "string");
}
/**
...
function checkOptionalBoolean(argument, name) {
    return checkOptionalArgumentFull(argument, name, isBoolean, "boolean");
}
...
function loopThroughList(list, callback, startIndex = 0, endIndex = list.length, step = 1, scope) {
    ...
    for (/** @description The current index. @type {Number} */
        ...
        listLength = list.length; index < endIndex; index += step) {
        /**
...
//            throw new Error(createStringFromTemplate(COMMON_TEXT_LIST.invalidStringArgumentCannotBeEmpty, "name"));
...
class UnknownError extends Error {
     ...
     * @constructs UnknownError
     * @since 1.0
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    constructor(message = "Unknown error: " + message) {
        super(message);
    }
}
/**
...
class IndexOutOfBoundsError extends Error {
     ...
     * @constructs IndexOutOfBoundsError
     * @since 1.0
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    constructor(message) {
...
/**
 * @description Defines an impossible error that can never be thrown under intended circumstances - including exceptions.
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
...
Object.prototype.clear = function () {
    for (/** @param {String} propertyName The current property name. */
        let propertyName in this)
...
class CharCode {
     ...
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    static isHighSurrogate = charCode => isNumber(charCode) &&
     ...
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    static isLowSurrogate = charCode => isNumber(charCode) &&
 ...
 * @throws {TypeError} If another string is not null or undefined andnot a string either.
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
...
/**
 * @description Multiplies this string by the number of count.
...
String.prototype.multiply = function (count) {
    ...
    if (multiplier % 1 === 0) {
        for (/** @description This string multiplied. @type {String} */
            let product = this; multiplier !== 0; multiplier--)
                product += product;
    ...
    for (multiplier = 0; multiplier < count - 1; multiplier++)
        product += this;
...
function* codePointGenerator(string) {
    ...
    for (/** @description The current index. @type {Number} */
        let index = 0; index < normalizedString.length; index++) {
        /**
...
String.prototype.loopThroughCodePoints = function (callback, scope) {
    ...
    for (/** @description The currrent code point. @type {Number} */
        const codePoint of codePointGenerator(this)) {
        /**
         * @description If the loop can continue.
         * @type {Boolean}
         */
...
String.prototype.loopThroughCharCodes = function (callback, startIndex, endIndex, step, scope) {
    return this.loop(character => callback(character.charCodeAt(0)), startIndex, endIndex, step, scope);
};
/**
...
String.prototype.forEachCharCode = function (callback, startIndex, endIndex, step, scope) {
    return this.forEach(character => callback(character.charCodeAt(0)), startIndex, endIndex, step, scope);
};
/**
...
class StringTemplateError extends Error {
     ...
     * @constructs StringTemplateError
     * @param {String} message The message.
 ...
 * @param {Array} parameterList The parameter values.
...
function createStringFromTemplate(templateString, ...parameterList) {
     ...
     * @type {String}
     */
    const escapeCharacter = '|';// "\u005C"; // TODO What to do about this?
    /**
 ...
 */
function getSingleValueFromString(templateString, value) {// TODO Remove the string errors.
    checkRequiredString(templateString, "templateString");
    checkRequiredString(value, "value");
    ...
    // TODO Add documentation
    const valueDoesntMatchTemplateErrorMessageTemplate = "The specified value doesn't match the specified template (@).";

    if (value.length < templateString.length)
        throw createStringFromTemplate(valueDoesntMatchTemplateErrorMessageTemplate, "the value is shorter than the template");
    ...
    if (valueIndex !== undefined) {
        if (valueIndex > 0 &&
            ...
            throw createStringFromTemplate(valueDoesntMatchTemplateErrorMessageTemplate, "mismatch at the beginning of the value");

        if (++valueIndex !== string.length &&
                !value.endsWith(string.substring(valueIndex, string.length)))
            throw createStringFromTemplate(valueDoesntMatchTemplateErrorMessageTemplate, "mismatch at the end of the value");
...
Array.prototype.merge = function (array) {
    ...
    for (/** @description The current item index. @type {Number} */
        let itemIndex = 0; itemIndex < array.length; itemIndex++)
...
// TODO Add documentation
function dissectCssColorStatement(cssStatement) {
    checkRequiredString(cssStatement, "cssStatement");

    // TODO Add documentation
    let endIndex = 0;
    // TODO Add documentation
    const checkCharCode = charCode => (charCode >= 0x41 &&
            charCode <= 0x5A) ||
            (charCode >= 0x61 &&
            charCode <= 0x7A);

    while (checkCharCode(cssStatement.charCodeAt(endIndex++)));

    if (--endIndex === 0)
        throw new SyntaxError("A name has been expected but none was found.");

    if (cssStatement[endIndex] !== '(')
        throw new SyntaxError(`Left parenthesis has been expected but character ${createUnicodeNotation(cssStatement.charCodeAt(endIndex))} has been found.`);
    ...
    const valueList = [];
    // TODO Add documentation
    const setValue = (startIndex, endIndex) => valueList.push(cssStatement.substring(startIndex, endIndex).trim());

    setValueList: for (
        // TODO Add documentation
        let index = endIndex + 1;
        index < cssStatement.length;
        index++) {
        // TODO Add documentation
        const character = cssStatement[index];

        switch (character) {
            case ',':
                if (commaIndex === undefined)
                    setValue(endIndex + 1, index);
                else
                    setValue(commaIndex + 1, index);
            ...
            case ')':
                setValue(commaIndex + 1, index);
                return {
                    name: cssStatement.substring(0, endIndex).toLowerCase(),
                    valueList: valueList
                };
        }
    }

    throw new SyntaxError("Right parenthesis has been expected but none was found.");
}
/**
...
class RgbColor {
     ...
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    static createHexStatement = colorNumber => checkRequiredNumber(colorNumber, "colorNumber").
            toString(16).
            toUpperCase().
            maskLeft("0", 2);
    /**
    ...
    // TODO Add documentation
    // TODO Doesn't the rgb() statement also support opacity?
    static createCssStatement = (redColorNumber, greenColorNumber, blueColorNumber, opacity) => `rgb(${RgbColor.check(redColorNumber)},${RgbColor.check(greenColorNumber)},${RgbColor.check(blueColorNumber)})`;
    // TODO Add documentation
    static readCssStatement = cssStatement => {
        if (String.isEmpty(cssStatement))
            return undefined;

        // TODO Add documentation
        const dissectedCssColorStatement = dissectCssColorStatement(cssStatement);
        // TODO Add documentation
        let rgbColor = {};

        switch (dissectedCssColorStatement.name) {
            case "rgb":
                if (dissectedCssColorStatement.valueList.length !== 3)
                    throw new ColorError(`Invalid RGB CSS statement found: three colors expected, ${dissectedCssColorStatement.valueList.length} found.`);

                break;
            case "rgba":
                if (dissectedCssColorStatement.valueList.length !== 4)
                    throw new ColorError(`Invalid RGBA CSS statement found: four colors expected, ${dissectedCssColorStatement.valueList.length} found.`);

            {
                // TODO Add documentation 
                const opacity = Number.parseFloat(dissectedCssColorStatement.valueList.pop());

                if (opacity !== 0)
                    rgbColor.opacity = opacity;
            }
            default:
                throw new ImpossibleError(`Unexpected color scheme name ("${dissectedCssColorStatement.name}") found, "RGB" or "RGBA" expected.`);
        }

        ["redColorNumber", "greenColorNumber", "blueColorNumber"].forEach((colorName, index) => rgbColor[colorName] = Number.parseInt(dissectedCssColorStatement.valueList[index]));
        return Object.freeze(rgbColor);
    };
    // TODO Add documentation
    static fromCssStatement = cssStatement => {
        // TODO Add documentation
        const colorStatementNameIndex = cssStatement.indexOf("(");

        if (colorStatementNameIndex < 0)
            throw "the specified CSS statement is invalid. It contains no CSS RGB or RGBA statement.";

        // TODO Add documentation
        const colorNumberList = getSingleValueFromString("rgb(@)", cssStatement).split(",").map(x => Number.parseInt(x.trim()));

        if (colorNumberList.length !== 3)
            throw `Invalid RGB CSS statement found: three colors expected, ${colorNumberList} found.`;

        return Object.freeze({
            redColorNumber: colorNumberList.shift(),
            greenColorNumber: colorNumberList.shift(),
            blueColorNumber: colorNumberList.shift()
        });
    };
    // TODO Add documentation
...
class ClientId {
    ...
    /**
     * @description Generates a client ID, which is a 32 alphanumeric characters long case-insensetive ID that is unique on the client side and should only be used on the client. The ID consists out of
     * randomly generated characters. Combined, both numeric values are represented using base 36 and totals 32 alphanumeric characters.
     * @returns {String} A new client ID.
     * @throws {String} If an unknown error occurs.
     * @public
     * @since 1.0
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    static generate() {
        ...
        return uncutClientId.length > ClientId.length ?
            uncutClientId.substring(uncutClientId.length - ClientId.length) :
            uncutClientId.length < ClientId.length ?
            uncutClientId.maskLeft("0", ClientId.length) :
            uncutClientId;
    }
    /**
     ...
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    static generateList(...names) {
        /**
         * @description The client ID list.
         * @type {Object}
         */
        const clientIdList = {};
        names.forEach(name => clientIdList[name] = ClientId.generate());
        return clientIdList;
    }
    /**
...
/**
 * @description Defines a static interface that represents read-only UUID64 features. A UUID64 is a 22 character long Base64 encoded (with in mind XML compatibility) UUID.
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
class Uuid64 {
     ...
     * @returns {Boolean}
     */
    static isValid = uuid64 => !isNullOrUndefined(uuid64) &&
            isString(uuid64) &&
...
class DomId {
    ...
    /**
     * @description Checks if the specified code point is a valid DOM ID code point.
     * @param {Number} codePoint The code point.
     * @param {Boolean} isStartingCodePoint If the code point is to be treated as being at the first index of the DOM ID.
     * @returns {Boolean} If the specified code point is a valid DOM ID code point.
     * @throws {TypeError} If the code point is not a number.
     * @throws {TypeError} if "isStartingCodePoint" is not a boolean.
     * @public
     * @since 1.0
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    static isValidCodePoint(codePoint, isStartingCodePoint = false) {
        checkRequiredBoolean(isStartingCodePoint, "isStartingCodePoint");

        if (isNullOrUndefined(codePoint))
            return false;

        if (!isNumber(codePoint))
            throw new TypeError(createStringFromTemplate(COMMON_TEXT_LIST.invalidArgumentDataTypeTemplate, "codePoint", "number"));


        if (!isBoolean(isStartingCodePoint))
            throw new TypeError(createStringFromTemplate(COMMON_TEXT_LIST.invalidArgumentDataTypeTemplate, "isStartingCodePoint", "boolean"));
        ...
        return (
                ...
                (
                    codePoint === 0x3A /* colon */ ||
                    codePoint === 0x2E /* full stop */ ||
                    codePoint >= 0x30 &&
                    codePoint <= 0x39 /* 0..1 */
                ) && !isStartingCodePoint
            );
    ...
    static isValidComponent (component, isFirstComponent) {
        ...
        return !component.isEmpty() &&
                component.loopThroughCodePoints((codePoint, index) => DomId.isValidCodePoint(codePoint, isFirstComponent &&
                index === 0));
    }
    /**
    ...
    static isValid(domId) {
        return !isNullOrUndefined(domId) &&
            ...
            domId.split(DomId.separator).loop(
                ...
                component.loopThroughCodePoints(
                /**
                 * @description Checks if the specified DOM ID character is valid.
                 * @param {String} codePoint The DOM ID code point.
                 * @param {Number} codePointIndex The index of the DOM ID code point.
                 * @returns {Boolean} If the specified DOM ID character is valid.
                 */
                (codePoint, codePointIndex) => DomId.isValidCodePoint(codePoint, componentIndex === 0 &&
                    codePointIndex === 0)));
    }
    /**
        ...
        domId.split(DomId.separator).loop(
            ...
            (component, componentIndex) => {
                ...
                component.loop(
                    /**
                     * @description Checks if the specified DOM ID character is valid.
                     * @param {String} character The DOM ID character.
                     * @param {Number} characterIndex The index of the DOM ID character.
                     * @returns {Boolean} If the specified DOM ID character is valid.
                     */
                    (character, characterIndex) => {
                        if (DomId.isValidCodePoint(character, componentIndex === 0 &&
                                characterIndex === 0))
                            throw new DomIdError(`The specified DOM ID contains an invalid character (${createUnicodeNotation(character)}) at index ${characterIndex}.`);
                    });
            });
    }
    /**
     * @description Creates a DOM ID with the specified components.
     * @argument {Array} componentList The components.
     ...
     * @since 1.0
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     * @example DomId.create("user", 43); // returns "user_43".
     */
     ...
     * @param {Array} parameterList The parameters.
     * @returns {String} A DOM ID.
     ...
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     * @see DomId.create()
     * @example DomId.createFromTemplate(["project", "row", null], 43); // returns "project_row_43".
     */
    ...
    static getIdList(template, domId) {
        ...
        for (/**
             ...
             * @type {Number}
             */
            let templateComponentIndex = 0; templateComponentIndex < domIdcomponentList.length; templateComponentIndex++) {
            /**
    ...
    static getLastId(template, domId) {
        ...
        for (/**
             ...
             * @type {Number}
             */
            let templateComponentIndex = 0; templateComponentIndex < domIdcomponentList.length - 1; templateComponentIndex++) {
            /**
             * @description The current template component.
             * @type {undefined|String|Number|Boolean}
             */
     ...
     * @since 1.0
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     * @example DomId.domIdMatchesTemplate(["a", "b", null, "c", null], "a_b_3_c_1343"); // returns true
     */
 ...
 * @throws {String} If multiple elements have been found with the specified DOM ID.
 * @throws {String} If the DOM ID has not been specified.
 * @public
 * @since 1.0
 * @author Manuel Milosavljević
...
HTMLDocument.prototype.getElementByDomId = function () {
     ...
     * @type {String}
     */
    const domId = DomId.create.apply(this, arguments);
    /**
...
HTMLDocument.prototype.hasElementWithDomId = function () {
     ...
     * @type {Element}
     */
    const element = document.getElementByDomId.apply(this, arguments);

    if (isNullOrUndefined(element))
        throw new Error(createStringFromTemplate(COMMON_TEXT_LIST.multipleElementsFoundWithOneIdTemplate, element.id)); // TODO Test this out.
...
HTMLElement.prototype.hasBodyParent = function () {
    for (/** @param {Element} parentElement The parent element. */
        let parentElement = this.parentElement; !!parentElement; parentElement = parentElement.parentElement)
 ...
 * @returns {undefined}
 */
Node.prototype.appendChildNodes = function (...nodes) {
    for (/** @param {Number} nodeIndex The current node index. */
        let nodeIndex = 0; nodeIndex < nodes.length; nodeIndex++)
        this.appendChild(nodes[nodeIndex]);
};
/**
...
HTMLElement.prototype.getText = function () {
    for (/** @description The current element. @type {HTMLElement} */
        let element = this; ; element = element.children.getFirst())
    ...
    new MutationObserver(mutationList =>
        mutationList.forEach(/** @param {MutationRecord} mutationRecord */ mutationRecord => {
            for (/** @param {Number} The current added-node index. */
                let addedNodeIndex = 0; addedNodeIndex < mutationRecord.addedNodes.length; addedNodeIndex++) {
                /**
...
function getItemFromListOnClick(mouseEvent) {
    ...
    for (/** @param {Element} element The current element (starting from the mouseEvent.target and moving up to the parent elements). */
 ...
 * @param {String|Array|DOMTokenList} classNames The class names to set.
 * @returns {undefined}
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
DOMTokenList.prototype.set = function (...classNames) { // TODO Update documentation
    this.clear();
    this.append.apply(this, classNames);
};
/**
 * @description Appends the specified class names.
 * @param {String|Array|DOMTokenList} classNames The class names to append.
 * @returns {undefined}
 * @throws {Error} If the class names argument is neither a string, Array or a DOMTokenList.
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
DOMTokenList.prototype.append = function (...classNames) { // TODO Update documentation
    ...
    if (classNames.length === 1) {
        ...
        if (isString(firstArgument))
            classNames = firstArgument.split("\u0020");
        else if (firstArgument instanceof DOMTokenList)
            classNames = firstArgument;
        else if (!(firstArgument instanceof Array))
            throw new Error(COMMON_TEXT_LIST.invalidArgument + quoteString("classNames") + " is neither a string or an instance of Array or DOMTokenList.");
    }
    ...
    classNames.forEach(/** @param {String} className A class name. */ className => domTokenList.add(className));
};
/**
...
DOMTokenList.prototype.indexOf = function (className) {
    if (!isNullOrUndefined(className) ||
        ...
        for (/** @param {Number} classNameIndex The current class name index. */
            let classNameIndex = 0; classNameIndex < this.length; classNameIndex++)
...
class CssTable {
    ...
    createRow(callback) {
        ...
        for (/** @description The current cell index.*/
            let cellIndex = 0; cellIndex < this.cellsPerRow; cellIndex++) {
            /**
...
CSSStyleDeclaration.prototype.setTransformTranslate = function (abscissa, ordinate) {
    ...
    this.transform = createStringFromTemplate("translate(@)", translateStatement);
};
/**
...
// TODO Add documentation
CSSStyleDeclaration.prototype.setBackgroundColor = function (redColorNumber, greenColorNumber, blueColorNumber) {
    this.backgroundColor = RgbColor.createCssStatement(redColorNumber, greenColorNumber, blueColorNumber);
};
// TODO Add documentation
CSSStyleDeclaration.prototype.getBackgroundColor = function () {
    return dissectCssRgbColorStatement(this.backgroundColor);
};
 ...
 * @type {String}
 */
QueryParameterList.BOOLEAN_TRUE_PARAMETER_VALUE = "t";
/**
 * @description The <code>false</code> value as a URL query parameter value.
 * @type {String}
 */
QueryParameterList.BOOLEAN_FALSE_PARAMETER_VALUE = "f";
/**
 * @description The <code>null</code> value as a URL query parameter value.
 * @type {String}
 */
QueryParameterList.NULL_PARAMETER_VALUE = "n";
 ...
 * @param {String} [title=APPLICATION_FULL_TITLE_DEFAULT] The title for the state.
 * @returns {undefined}
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
QueryParameterList.clearUrlQueryParameterList = (title = APPLICATION_FULL_TITLE_DEFAULT) => history.pushState(null, title, location.pathname);
/**
...
/**
 * @description Checks if a locale has been saved in the session storage.
 * @returns {Boolean} If a locale has been saved in the session storage.
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
function hasLocaleInSession() {
    return !sessionStorage.isPropertyNullOrUndefined("languageCode") &&
        !sessionStorage.isPropertyNullOrUndefined("countryCode");
}
/**
 * @description Returns the locale that is saved in the session storage.
 * @returns {Locale} The locale that is saved in the session storage.
 * @throws {Error} If no locale in the session storage has been found.
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
function getLocaleFromSession() {
    if (hasLocaleInSession())
        return new Locale(sessionStorage.languageCode, sessionStorage.countryCode);

    throw new Error("No locale found.");
}
/**
 * @description Returns the locale that is either stored in the session storage or specified in options.js. This is the default locale.
 * @returns {Locale} The default locale that is specified options.js.
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
 */
function getLocale() {
    return hasLocaleInSession() ?
        new Locale(sessionStorage.languageCode, sessionStorage.countryCode) :
        new Locale(DEFAULT_LANGUAGE_CODE, DEFAULT_COUNTRY_CODE);
}
/**
 * @description Deletes the locale that is stored in the session storage.
 * @returns {undefined}
 * @public
 * @since 1.0
 * @author Manuel Milosavljević <manuel@synergystructure.com>
...
function deleteLocale() {
    delete sessionStorage.languageCode;
    delete sessionStorage.countryCode;
}
//</editor-fold>
...
function CustomEventTarget(domId) {
     ...
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    this.dispatchEvent = (eventName, ...parameters) => {
        checkEventName(eventName);
        ...
        if (!isNullOrUndefined(callbackList))
            for (/** @param {Number} callbackListIndex The current callback list index. */
                let callbackListIndex = 0; callbackListIndex < callbackList.length; callbackListIndex++) {
                /**
                 * @description If the loop can continue.
                 * @type {Boolean}
                 */
                const canContinue = callbackList[callbackListIndex].apply(_customEventTarget, parameters);
        ...
        if (eventTargetList.isEmpty()) {
            ...
            target.addEventListener(eventName, function () {
                ...
                for (/** @description The current event target list index. @type {Number} */
                    let eventTargetListIndex = 0; eventTargetListIndex < eventTargetList.length; eventTargetListIndex++) {
                    /**
...
class WebService {
    /**
     * @description WebService constructor.
     * @param {String} name A name for the web service.
     ...
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    setCredentials(sessionId, tokenId) {
     ...
     * @param {FormData} [parameterList] The request parameters.
     * @param {WebService.OnRequestReadyCallback} [onReady] The method to be executed when a HTTP response is received and ready to be processed.
     * @param {WebService.OnRequestExceptionCallback} [onException] The method to be executed when an exception is caught during an HTTP request.
     * @param {WebService.OnRequestAbortCallback} [onAbort] The method to be executed when the HTTP request has been aborted.
     * @param {WebService.OnRequestProgressCallback} [onProgress] The method to be executed upon data transfer progress.
     * @returns {XMLHttpRequest} An instance of XMLHttpRequest.
     * @throws {String} If the browser doesn't support CORS (and "onException" is not set).
     * @public
     * @since 1.0
     * @author Manuel Milosavljević <manuel@synergystructure.com>
     */
    static createRequest(method, url, mediaType, parameterList, onReady, onException, onAbort, onProgress) {
        /**
        ...
        checkRequiredString(method, "method");
        checkRequiredString(url, "url");
        checkOptionalString(mediaType, "mediaType");
        checkOptionalArgument(parameterList, "parameterList", FormData);
        checkRequiredArgument(onReady, "onReady", Function);
        checkOptionalArgument(onException, "onException", Function);
        checkOptionalArgument(onAbort, "onAbort", Function);
        checkOptionalArgument(onProgress, "onProgress", Function);
        ...
        try {
            ...
            // TODO Add parameter "requestTimeOut".
    //        if (!WEB_SERVICE_CONFIGURATION.isPropertyNullOrUndefined("requestTimeOut")) {
    //            request.timeout = WEB_SERVICE_CONFIGURATION.requestTimeOut;
    ...
    //        }
            ...
            if (isNullOrUndefined(parameterList))
                ...
                else if (!(parameterList instanceof FormData))
...
class BoxAndPicker {
    // TODO Add documentation
    constructor(id, value, parent, isReadOnly, requirementStatus, isHidden, firstScollableParent) {
        // TODO Add documentation
        function showPicker() {
        ...
        DomId.check(id, false);
        checkRequiredArgument(parent, "parent", HTMLElement);
        checkRequiredBoolean(isReadOnly, "isReadOnly");
        setRequirement(requirementStatus);
        checkRequiredBoolean(isHidden, "isHidden");
        checkRequiredArgument(firstScollableParent, "firstScollableParent", HTMLElement);

        this.boxFrame = parent = createDivisionElement(parent);
        this.boxFrame.classList.set(COMMON_CSS_CLASS_NAME_LIST.boxFrame);
        this.boxFrame.dataset.componentId = ClientId.generate();
        this.box = createTextBox(id, parent, value, isReadOnly, isRequired, isHidden);
        this.box.classList.set(COMMON_CSS_CLASS_NAME_LIST.box, COMMON_CSS_CLASS_NAME_LIST.dateBox);

        // TODO Add documentation
        const boxButton = createDivisionElement(this.boxFrame);
        boxButton.classList.set(COMMON_CSS_CLASS_NAME_LIST.icon);

        // TODO Add documentation
        const boxButtonImage = createDivisionElement(boxButton);
        boxButtonImage.classList.set(COMMON_CSS_CLASS_NAME_LIST.vectorImage);
        boxButton.style.width = createPixelStatement(boxButtonImage.getBoxWidth() /* box button icon */);
        boxButton.style.height = createPixelStatement(this.box.getBoxHeight() - boxButton.getOuterHeight());
        this.box.style.width = `calc(${ONE_HUNDRED_PERCENT_STATEMENT} - ${createPixelStatement(boxButton.getBoxWidth())})`;

        this.box.addEventListener(COMMON_EVENT_TYPE_LIST.click, boxOnClick);
        boxButton.addEventListener(COMMON_EVENT_TYPE_LIST.click, boxOnClick);
    }
    // TODO Add documentation
    checkPosition() {
...
// TODO Add documentation
function checkPickerPosition(box, boxFrame, picker, clearer, fullscreenResizer) {
    picker.style.left = "";
    picker.style.top = "";
    picker.style.height = "";
    picker.style.width = "";
     ...
     * @type {Number}
     */
    const pickerBoxWidth = picker.getBoxWidth();

    if ((pickerBoxWidth + (pickerBoxWidth / 100) * 80) >= screenPixelSize.width) {
        picker.style.left = createPixelStatement(0);
        picker.style.top = createPixelStatement(0);
        picker.style.width = createPixelStatement(document.body.getWidth());
        picker.style.height = createPixelStatement(document.body.getHeight());
        picker.style.border = "none";
        picker.style.boxShadow = "none";

        if (!isNullOrUndefined(fullscreenResizer))
            fullscreenResizer();
     ...
     * @type {Number}
     */
    const comboBoxFrameWidth = boxFrame.getBoxWidth();
    /**
     * @description The size of the combo box and its position relative to the viewport.
     * @type {DOMRect}
     */
    const comboBoxClientRectangles = box.getBoundingClientRect();
}

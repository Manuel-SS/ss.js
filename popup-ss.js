...
/* global CustomEventTarget, HTMLElement, HTMLBodyElement, HTMLDivElement, COMMON_CSS_CLASS_NAME_LIST, ClientId */
...
function FloatingPopupManager() { // TODO Add @throw tags.
    ...
    function getRegistrationIndex(hotspotId) {
        for (/** @description The current registration index. */
            let registrationIndex = 0;
            registrationIndex < _registrations.length;
            registrationIndex++) 
        {
            // TODO Add documentation
    ...
    function getHotspotByName(startElement, endElement, name) {
        for (/** @param {Element} element The current element. */
            ...
            !!element &&
                element !== endElement.parentElement;
            element = element.parentElement)
    ...
    function getHotspotByClassNames(startElement, endElement, classNames) {
        checkElement: for (/** @param {Element} element The current element. */
            let element = startElement;
            !!element &&
                element !== endElement.parentElement;
            element = element.parentElement)
            if (element.classList.length >= classNames.length) {
                for (/** @description The current class name index. @type {Number} */
    ...
    this.getRegisteredFloatingPopup = hotspotId => {
        ...
        for (/** @description The current registration index. @type {Number} */
            let registrationIndex = 0;
            registrationIndex < _registrations.length;
            registrationIndex++) {
            // TODO Add documentation

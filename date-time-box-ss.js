...
function loopThroughHoursOfDay(callback) {
    ...
    for (/** @description The current hour. @type {Number} */
        let hour = 0,
        /** @description The create-hour-notation method. @type {Function} */
        createHourNotation = ("uses24HourClock" in sessionStorage ?
...
/**
 * @description Creates an HTMLInputElement (the date box) as a control for selecting/entering a date - with or without a time - value based on ISO 8601.
 * @param {String} [id] The ID of the element.
 * @param {HTMLElement|String|Boolean} parent The parent node to which to append the element, if parent is an instance of HTMLElement. If the parent is a string, it will be used as the id of the
 * parent node. If parent is "true" or not set, the element will be appended to the document body. If the parent is "false" it will not be appeneded to any node.
 ...
 * @param {HTMLInputElement} [isHidden] If the input element must be hidden from the user.
 * @param {Element} [firstScrollableParent=parent] The first scrollable parent of the date box.
 * @returns {HTMLInputElement} The date box.
...
function showDateBox(id, parent, value, withTime = false, isReadOnly = false, isRequired = false, isHidden = false, firstScrollableParent = parent) {
    ...
    */
    ...
    function showLocaleDateTimeBox() {
        ...
        dateBox.value = new Intl.DateTimeFormat(document.documentElement.lang, optionList).format(selectedDateTime);
    }
    /**
    ...
    //        createSpanElement(dateHeader, "\xA0" + dateSeparator);
            ...
            if (datePickerStyle === 1) {
                ...
                for (// TODO Add documentation
                    let dayOfMonth = 1,
                    lastDayofMonth = selectedDateTime.getLastDayofMonth();
...
function test() {
    ...
    LOCALIZED_TEXTS_MAP = new Map();
    LOCALIZED_TEXTS_MAP.set(textNames.defaultDateFormat, "YYYY-MM-DDTHH:MM:SS");
    LOCALIZED_TEXTS_MAP.set(textNames.weekNumber, "Week number");
    LOCALIZED_TEXTS_MAP.set(textNames.weekHashtag, "W#");
    LOCALIZED_TEXTS_MAP.set(textNames.year, "Year");
    LOCALIZED_TEXTS_MAP.set(textNames.month, "Month");
    LOCALIZED_TEXTS_MAP.set(textNames.invalidYearSpecified, "Invalid year specified");
    LOCALIZED_TEXTS_MAP.set(textNames.today, "Today");
    LOCALIZED_TEXTS_MAP.set(textNames.hours, "Hours");
    LOCALIZED_TEXTS_MAP.set(textNames.minutes, "Minutes");
    LOCALIZED_TEXTS_MAP.set(textNames.seconds, "Seconds");
    LOCALIZED_TEXTS_MAP.set(textNames.milliseconds, "Milliseconds");
    LOCALIZED_TEXTS_MAP.set(textNames.invalidTimeSpecifiedCurrentTimeWillBeUsed, "Invalid time has been specified. The current time will be used.");
    showDateBox(undefined, document.body, undefined, true, false, false, false, document.body);
}
